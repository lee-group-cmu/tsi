...
class GAMWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self):
        """
        gam_formula_constructor: A CALLABLE that returns the formula (not the formula itself)
        lam: Pre-optimized lambda value(s)
        """
    ...
    def predict_proba(self, X):
        probs = self.gam_model_.predict_proba(X)
...
def main(experiment_id):
    ...
    B_PRIME = 10_000  # num simulations to estimate critical values
